...
def _resolve_palette(palette=None):
    if isinstance(palette, QPalette):
        return palette

    app = QApplication.instance()
    if app and app.activeWindow():
        return app.activeWindow().palette()

    return app.palette() if app else QPalette()


def _refresh_palette_colors(palette=None):
    """Keep palette-derived colors in sync with the current Krita theme."""
    global highlight
    global background
    global alternate
    global inactive_text_color
    global active_text_color
    ...
    inactive_text_color = palette.color(
        QPalette.ColorGroup.Disabled, QPalette.ColorRole.WindowText
    ).name()[1:]
    active_text_color = palette.color(
...
def refreshThemeStyles(palette=None):
    _refresh_palette_colors(palette)
    buildNuToolsTheme()
    buildFlatTheme()
